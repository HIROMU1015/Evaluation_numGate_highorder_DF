...
def load_matrix_files(n_qubits, t, ham_name, num_w):
    ...
    matrix_files = [name for name in os.listdir(directory_path) if name.startswith("matrix_")]
    matrix_paths = sorted(matrix_files, key=lambda s: int(os.path.splitext(s)[0].split("_")[1]))
...
def folder_maker_multiprocessing_values(t_values, jw_hamiltonian, n_qubits, ham_name, num_w):
    """t ごとにフォルダと e^{iHt} 分解を並列生成。32 並列を固定（挙動不変）。"""
    workers = 32
    partition_size = (len(t_values) + workers - 1) // workers
    t_partitions = [t_values[i * partition_size:(i + 1) * partition_size] for i in range(workers)]

    if num_w is not None:
        task_args = [(jw_hamiltonian, t, n_qubits, ham_name, num_w, i) for i, t in enumerate(t_partitions)]
        with Pool(processes=workers) as pool:
            pool.starmap(w_trotter_folder_maker_multi, task_args)
    else:
        task_args = [(jw_hamiltonian, t, n_qubits, ham_name, i) for i, t in enumerate(t_partitions)]
        with Pool(processes=workers) as pool:
            pool.starmap(normal_trotter_folder_maker_multi, task_args)
    print('done')


def normal_trotter_folder_maker_multi(jw_hamiltonian, t_list, n_qubits, ham_name, core_num):
...
def w_trotter_folder_maker_multi(jw_hamiltonian, t_list, n_qubits, ham_name, num_w, core_num):
    """重み付き（高次）PF のフォルダ生成。"""
    ...
    for t in t_list:
        ...
        if m == 1:
            ...
            continue
        # w_m
        idx = S_2_trotter_left(ham_list, t, n_qubits, w_list[m - 1], w_list[m - 2], folder_path, idx)
        # w_{m-1} ~ w_1
        for i in reversed(range(1, m - 1)):
            idx = S_2_trotter(ham_list, t, n_qubits, w_list[i], w_list[i - 1], folder_path, idx)
        # w_0 ~ w_{m-1}
...
def multi_parallel_sparse_matrix_multiply_recursive(initial_folder_path, file_names, num_partitions):
    ...
    while len(file_names) >= 2:
        ...
        partitions = [file_names[i * partition_size:(i + 1) * partition_size] for i in range(num_partitions)]
        with Pool(processes=num_partitions) as pool:
            results = pool.starmap(
                sparse_matrix_multiply_from_folder,
                [(core, current_folder_path, part) for core, part in enumerate(partitions)],
            )
...
def error_cal_multi(t_list, terms_list, ori_vec, E, num_eig):
    ...
    for t, terms in zip(t_list, terms_list):
        ...
        # シフトは E の1点のみ（既存挙動）
        sigma = np.exp(1j * E * t)
        eigenvalues = eigs(terms, sigma=sigma, k=num_eig, v0=ori_vec, return_eigenvectors=False)
...
def trotter_error_plt(
    ...
    neg_t_values = [-1.0 * t for t in t_values]  # 互換のため保持（未使用でも挙動不変）
    ...
    make_folder_t = []
    for t in t_values:
        ham_dir = os.path.join(parent_dir, "matrix", f"{ham_name}_Operator_{num_w_dir}", f"{t}")
    ...
    if len(make_folder_t) > 0:
        folder_maker_multiprocessing_values(make_folder_t, hamiltonian, n_qubits, ham_name, num_w)
    ...
    for t in t_values:
        ...
        if not os.path.exists(term_dir_path):
            ...
            term = multi_parallel_sparse_matrix_multiply_recursive(folder_path, file_path, 32)
    ...
    if storage is True:
        data = {"expo": slope, "coeff": coeff}
        target_path = f"{ham_name}_Operator_{num_w_dir}" if num_w is not None else f"{ham_name}_Operator_normal"
        save_data(target_path, data)

    if avestrage is True:
        target_path = f"{ham_name}_Operator_{num_w_dir}_ave" if num_w is not None else f"{ham_name}_Operator_normal_ave"
        save_data(target_path, ave_coeff)
    print(f"average_coeff:{ave_coeff}")
...
def trotter_error_plt_qc(
    ...
    plt.plot(t_values, error_list_ph, marker="s", linestyle="-", label="Diagonalization")
    plt.legend()
    plt.show()
    ...
    plt.xlabel("Time", fontsize=15)
    plt.ylabel("Eigenvalue error [Hartree]", fontsize=15)
    plt.plot(t_values, error_list_pertur, marker="s", linestyle="-", label="Perturbation", color="green")
    plt.legend()
    plt.show()
